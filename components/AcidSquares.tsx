...
|------|------|---------|-------------|
| color1 | string | '#5227FF' | Violet core color used for the dim depths of the corridor. |
| color2 | string | '#A855F7' | Pink mid-tone color used for the glowing crystal faces. |
| color3 | string | '#FFFFFF' | White highlight color used for the hottest edges. |
| detail | "low" | "medium" | "high" | 'medium' | Raymarch quality tier (20 / 32 / 48 steps). Higher looks richer but costs more. |
| speed | number | 0.7 | Speed of the forward drift through the lattice. |
| waveDepth | number | 1 | How far the view gently drifts in and back out as the corridor breathes. 0 holds it still. |
| zoom | number | 1.3 | Field of view into the corridor. Higher zooms in. |
| density | number | 10.0 | How tightly the boxes of the lattice are packed. |
| glow | number | 1.0 | Gain applied to the accumulated light along each ray. |
| exposure | number | 2700 | Tone-mapping exposure. Higher values darken the render. |
| spread | number | 0.3 | Controls how the ray step grows each iteration; strongly shapes the look. |
| stepSize | number | 0.002 | Minimum ray step. Larger values thin out the glow. |
| colorShift | number | 0 | Rate of the gentle brightness shimmer cycling over time. |
| contrast | number | 1 | Contrast applied around the mid-tones of the final image. |
| brightness | number | 1.0 | Overall brightness multiplier before tone mapping. |
| opacity | number | 1.0 | Overall opacity of the effect over the transparent background. |
| mouseInteraction | boolean | true | Enables a soft depression that sinks the lattice downward around the pointer. |
| mouseStrength | number | 0.1 | How deeply the cursor presses the lattice down inside the affected area. |
| mouseRadius | number | 0.35 | Size of the area around the pointer that collapses, as a fraction of the view height. |
| blur | number | 0 | Softens the whole effect with a post-process Gaussian blur. 0 is crisp; 1 is a soft haze. |
| grain | boolean | true | Adds a faint animated film grain over the final image for a subtle analog texture. |
